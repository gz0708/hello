...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
   ...
   """
    ...
    set1 = set()
    set2 = set()
    zd = read_csv_as_nested_dict(gdpinfo["gdpfile"],gdpinfo["country_name"],gdpinfo["separator"],gdpinfo["quote"])
    jie = reconcile_countries_by_name(plot_countries,zd)
    for k,v in jie[0].items():
        b = 0
        for i in range(gdpinfo["min_year"],gdpinfo["max_year"]+1):
            if zd[v][str(i)] =='':
                ...
                b = b + 1
        if zd[v][year]!='':
            gdp[k]=math.log10(float(zd[v][year]))
            set2.add(k)
        if b == 0:
            set1.add(k)
        else:
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    """
    jie1 = build_map_dict_by_name(gdpinfo,plot_countries,year)
    dict1 = jie1[0]
    set1 = jie1[1]
    set2 = jie1[2]
...
def test_render_world_map(year):  #测试函数
    ...
    render_world_map(gdpinfo, pygal_countries, year, "isp_gdp_world_name_2000.svg")
